...
> I'd fix the one issue I flagged as the biggest remaining problem: the unique index including soft-deleted rows. Right now a teacher can't correct a wrong entry — soft-deleting it and re-recording hits a constraint violation because the deleted row still holds its slot. The fix is a single migration adding WHERE deleted_at IS NULL to make it a partial index, and a regression test to prove the re-record workflow actually works. That's maybe twenty minutes of work but it closes the gap between what the soft-delete model promises and what it delivers.
